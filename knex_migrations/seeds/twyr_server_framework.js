...
exports.seed = function(knex, Promise) {
	return knex.raw('SELECT id FROM modules WHERE name = ? AND parent_id IS NULL', ['twyr-server'])
		...
		return Promise.all([
			...
			.then(function(parentId) {
				...
				return Promise.all([
					...
					knex("modules").insert({ 'parent_id': parentId, 'type': 'service', 'name': 'api-service', 'display_name': 'API Service', 'description': 'The Twy\'r Server API Service - allows modules to expose interfaces for use by other modules without direct references to each other' }),
